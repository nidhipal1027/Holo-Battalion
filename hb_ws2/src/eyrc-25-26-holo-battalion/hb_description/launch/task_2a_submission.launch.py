...
def generate_launch_description():
    ...
    robot_state_publisher_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(launch_file_dir, 'rsp.crystal.launch.py')
        ),
    ...
    spawn_holonomic_bot = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(launch_file_dir, 'spawn_hb_crystal.launch.py')
        )
    )

    box_spawner = Node(
            namespace='eyantrasim', package='hb_description',
            executable='box_placer_2a', output='screen')
